...
def set_up_home(chrani_bot, source_player, target_player, command):
    try:
        ...
        chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "Home is where your hat is!", chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("success"))
...
def protect_inner_core(chrani_bot, source_player, target_player, command):
    try:
        ...
        if player_home_exists and location_object.set_protected_core(True):
            ...
            response_messages.add_message(message, True)
            chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "your home is now protected!", chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("success"))
...
def unprotect_inner_core(chrani_bot, source_player, target_player, command):
    try:
        ...
        if player_home_exists and location_object.set_protected_core(False):
            chrani_bot.locations.upsert(location_object, save=True)
            chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "your home is now unprotected!", chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("success"))
            chrani_bot.socketio.emit('update_leaflet_markers', chrani_bot.locations.get_leaflet_marker_json([location_object]), namespace='/chrani-bot/public')
        else:
            message = "something went wrong :("
            response_messages.add_message(message, False)
    ...
    try:
        ...
        try:
            ...
            if location_object.player_is_inside_boundary(target_player):
                chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "eh, you already ARE home oO".format(target_player.name), chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("warning"))
            else:
            ...
            chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "You seem to be homeless".format(target_player.name), chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("warning"))
    ...
    try:
        ...
        if p:
            ...
            try:
                ...
                chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "teleportplayer", target_player, location_object=location_object)
                chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "You went to visit {}'s home".format(player_object_to_port_to.name), chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("standard"))
                chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", player_object_to_port_to, "{} went to visit your home!".format(target_player.name), chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("warning"))
            except KeyError:
                chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "Could not find {}'s home".format(player_steamid), chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("warning"))
                pass
    ...
    try:
        ...
        if player_home_exists:
            ...
            if set_radius is True:
                ...
                chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "you given range ({}) seems to be invalid. Are you inside your home area?".format(int(location_object.warning_boundary * 2)), chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("warning"))

        if player_home_exists and coords_are_valid:
            chrani_bot.telnet_observer.actions.common.trigger_action(chrani_bot, "pm", target_player, "your private area ends here and spans {} meters ^^".format(int(location_object.warning_boundary * 2)), chrani_bot.dom.get("bot_data").get("settings").get("color_scheme").get("success"))
