    ...
    try:
        ...
        if p:
            ...
            try:
                ...
                self.bot.locations.upsert(location_object, save=True)
                self.tn.say("{} called their home {}".format(player_object.name, location_object.description), color=self.bot.chat_colors['background'])
                return True

            except KeyError:
                self.tn.send_message_to_player(player_object, "{} can not name that which he does not have!".format(player_object.name), color=self.bot.chat_colors['warning'])
    except Exception as e:
        logger.error(e)
        pass
...
def goto_playerhome(self, command):
    try:
        player_object = self.bot.players.get(self.player_steamid)
        p = re.search(r"goto playerhome (.+)", command)
        if p:
            ...
            try:
                ...
                self.tn.teleportplayer(player_object, location_object)
                self.tn.say("{} went to visit {}".format(player_object.name, player_object_to_port_to.name), color=self.bot.chat_colors['background'])
            except KeyError:
                pass
    except Exception as e:
        logger.error(e)
...
actions_home.append(("startswith", "goto playerhome", goto_playerhome, "(self, command)", "home"))
